...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        elif self.epsilon > 0.01 and self.epsilon > random.random(): # to satisfy epsilon probability of taking random action
            action = random.choice(self.valid_actions)
        else:
            actions = []
            maxQ_state = self.get_maxQ(state)
            for action in self.Q[state]: # account for equal Q values
                if maxQ_state == self.Q[state][action]:
